package com.baker.learning.javabasics.designpattern.creational.singleton.staticinnerclass;

/**
 * @author ylp
 * @Description 单例模式之静态内部类实现
 * @date 2020/7/2 12:19 上午
 * @see
 */
public class StaticInnerClassSingleton {
     ...
     * ，只有当getInstance()方法第一次被调用时，才会去初始化INSTANCE,第一次调用getInstance()
     * 方法会导致虚拟机加载SingleTonHoler类，这种方法不仅能确保线程安全，也能保证单例的唯一性，同时也延迟了单例的实例化。
     *
     ...
     *          new一个关键字或者一个实例化对象时、读取或设置一个静态字段时
     ...
     * 取的都是同一个INSTANCE对象，而不用去重新创建。当getInstance()方法被调用时，SingleTonHoler才在SingleTon的
     * 运行时常量池里，把符号引用替换为直接引用，这时静态对象INSTANCE也真正被创建，然后再被getInstance()
     * 方法返回出去，这点同饿汉模式。那么INSTANCE在创建过程中又是如何保证线程安全的呢？
